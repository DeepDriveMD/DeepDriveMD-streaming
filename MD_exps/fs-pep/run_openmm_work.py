...
parser.add_argument("-d", "--dir", dest="d", help="current directory")
...
if args.d:
    current_dir = os.path.abspath(args.d)
else:
    current_dir = None
...
def prepare(current_dir):
    old_dir = os.getcwd()
    os.chdir(f'{current_dir}/MD_exps/fs-pep')
    time_stamp = int(time.time())
    run_dir = f'omm_runs_{time_stamp}'
    os.mkdir(run_dir)
    os.chdir(run_dir)
    subprocess.getstatusoutput('cp ../../../adios.xml .')
