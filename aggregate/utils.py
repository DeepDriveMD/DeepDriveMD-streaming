...
def format_cm(cm):
    cm1 = cm.reshape([1,cm.shape[0]])
    cm2 = np.array([triu_to_full(d) for d in cm1])
    pad_f = lambda x: (0,0) if x%2 == 0 else (0,1)
    padding_buffer = [(0,0)]
    for x in cm2.shape[1:]:
        padding_buffer.append(pad_f(x))
    cm3 = np.pad(cm2, padding_buffer, mode = 'constant')
    cm4 = cm3.reshape(cm3.shape + (1,))
    return cm4[0]
...
def adios_cm_to_cvae(adios_file_list): 
    #cm_all = np.hstack(adios_cm_data_lists)
    #big_list_of_data = []
    big_cms = []
    big_positions = []
    for fn in adios_file_list:
        small = read_adios_file(fn)
        #small_list_of_data = read_adios_file(fn)
        big_cms += small[0]
        big_positions += small[1]
        #big_list_of_data += small_list_of_data
